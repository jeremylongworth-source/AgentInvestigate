...
Assesses camera coverage gaps, privacy limits, source constraints, coverage assumptions, and qualified-review needs. This is a `REGULATED` security systems and technology skill for professional security support.
...
- User asks to define, analyze, assess, identify, or prepare camera coverage gap.
...
- Supplied facts relevant to camera coverage gap, including access control, video surveillance, intrusion detection, alarm monitoring, event analysis, coverage, failures, and requirements where applicable.
...
- Read `references/assess-camera-coverage-gap-reference.md` when preparing camera coverage gap outputs.
