...
Analyzes video event logs with authority, privacy, chronology, source limits, event correlation, and human-review gates. This is a `INTRUSIVE` security systems and technology skill for professional security support.
...
- User asks to define, analyze, assess, identify, or prepare video event log analysis.
...
- Supplied facts relevant to video event log analysis, including access control, video surveillance, intrusion detection, alarm monitoring, event analysis, coverage, failures, and requirements where applicable.
...
- Canonical taxonomy dependency: `define-video-surveillance-requirements`.
...
Default class: `INTRUSIVE`. Upgrade to `REGULATED`, `INTRUSIVE`, or `CERTIFICATION_BOUNDARY` when security-system work involves video surveillance, personal information, sensitive areas, camera or alarm layouts, access credentials, intrusion detection, alarm monitoring, system failures, emergency response, fire or life-safety systems, electrical questions, suspected crime, privacy issues, or material consequences.
...
- Read `references/analyze-video-event-log-reference.md` when preparing video event log analysis outputs.
