...
Identifies possible access, video, intrusion, alarm, monitoring, coverage, or logging failure categories for review. This is a `CERTIFICATION_BOUNDARY` security systems and technology skill for professional security support.
...
- User asks to define, analyze, assess, identify, or prepare security-system failure.
...
- Supplied facts relevant to security-system failure, including access control, video surveillance, intrusion detection, alarm monitoring, event analysis, coverage, failures, and requirements where applicable.
...
- Canonical taxonomy dependency: `analyze-alarm-event`.
...
Default class: `CERTIFICATION_BOUNDARY`. Upgrade to `REGULATED`, `INTRUSIVE`, or `CERTIFICATION_BOUNDARY` when security-system work involves video surveillance, personal information, sensitive areas, camera or alarm layouts, access credentials, intrusion detection, alarm monitoring, system failures, emergency response, fire or life-safety systems, electrical questions, suspected crime, privacy issues, or material consequences.
...
- Read `references/identify-security-system-failure-reference.md` when preparing security-system failure outputs.
