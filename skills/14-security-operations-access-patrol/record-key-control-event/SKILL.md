...
Records key-control events with custody, authorization, transfer, exception, and supervisor review details. This is a `REGULATED` security operations, access, and patrol skill for professional security support.
...
- User asks to review, build, plan, document, log, verify, triage, record, prepare, or identify key-control event.
...
- Supplied facts relevant to key-control event, including dates, times, locations, people or roles, access points, alarm zones, patrol points, visitor details, key identifiers, notifications, actions taken, and open issues where applicable.
...
- Canonical taxonomy dependency: `verify-access-event`.
...
- Read `references/record-key-control-event-reference.md` when preparing key-control event outputs.
