...
Checks timestamp source, format, timezone, chain context, conflicts, and unresolved disputes. This is a `ROUTINE` evidence and chain-of-custody skill for professional investigation and security support.

## Triggers

- User asks to create, classify, record, assess, summarize, identify, track, compare, verify, map, or escalate evidence timestamp review.
...
- Supplied evidence item details, source, original/copy status, custodian, transfer, timestamp, storage, or continuity record relevant to evidence timestamp review.
...
- Canonical taxonomy dependency: `record-evidence-source`.
...
- Read `references/verify-evidence-timestamp-reference.md` when preparing evidence timestamp review outputs.
