...
python "$PSScriptRoot\validate-specialization-roadmap.py" --repo-root "$RepoRoot"
if ($LASTEXITCODE -ne 0) {
    exit $LASTEXITCODE
}
...
python "$PSScriptRoot\validate-public-release-distribution.py" --repo-root "$RepoRoot"
if ($LASTEXITCODE -ne 0) {
    exit $LASTEXITCODE
}
